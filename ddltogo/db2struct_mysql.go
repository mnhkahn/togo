...
func generateMysqlTypes(obj []map[string]string, depth int, jsonAnnotation bool, gormAnnotation bool, gureguTypes bool) string {
	structure := "struct {"

	keys := make([]string, 0, len(obj))
	tempMap := make(map[string]map[string]string, len(obj))
	for _, o := range obj {
		key := o["name"]
